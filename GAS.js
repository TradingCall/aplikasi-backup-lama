// ============================================================
// GACOR HUNTER V5.0 - EOD SCANNER & MARKET OUTLOOK BACKEND
// Google Apps Script - SYNCED WITH FRONTEND V5.0 SCORING
// ============================================================
...
const CONFIG = {
  MIN_PRICE: 50,
  MIN_AVG_DAILY_VALUE: 1000000000, // 1 Miliar
  MIN_VOLUME_EMA5: 1000000, // 1 Juta Lembar (10.000 Lot)
  RSI_PERIOD: 14,
  TOP_N: 10,
  RANGE: "6mo", // 6 bulan cukup untuk ambil EMA99 dan historical data
  INTERVAL: "1d",
  USER_AGENT: "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120 Safari/537.36",
  BATCH_SIZE: 25, // DIPERBAIKI: Turunkan dari 100 ke 25 untuk mencegah error 429 Yahoo Finance
  BATCH_SLEEP_MS: 1000
};

// ============================================================
// 2. WATCHLIST (UNIVERSE SAHAM IDX)
// ============================================================
const WATCHLIST = [
...
// ============================================================
// 3. V5.0 HELPER MATH & INDICATORS (SYNCED WITH FRONTEND)
// ============================================================
...
function nearestBelow(values, price) {
  const x = values.filter(v => Number.isFinite(v) && v < price);
...
function buildPlan(price, direction, setup, supports, resistances, atr) {
  // DIPERBAIKI: Mengoptimalkan return jika market BEARISH agar hemat eksekusi.
...
function generateMarketOutlook() {
  ...
  try {
    ...
    // DIPERBAIKI: Optional Chaining untuk cegah TypeError
    ...
    const outlookData = {
      ...
      summary: trend === "BULLISH" 
        ...
        : trend === "BEARISH" 
        ? "IHSG bergerak di bawah MA20, tren saat ini BEARISH. Market rawan koreksi lanjutan. Kurangi size entry, perketat Stop Loss, dan fokus pada saham defensif atau yang memiliki sentimen kuat melawan arus market."
        : "IHSG berkonsolidasi (SIDEWAYS) di sekitar area MA20. Gunakan strategi Buy on Support & Sell on Resistance. Hindari FOMO pada saham yang sudah rally panjang."
    };
    
    PropertiesService.getScriptProperties().setProperty("MARKET_OUTLOOK", JSON.stringify(outlookData));
  } catch(e) {
    Logger.log("Gagal generate Market Outlook: " + e.message);
  }
}

// ============================================================
// 5. MAIN EOD SCAN ENGINE (V5.0 LOGIC)
// ============================================================
function runEODScan() {
  ...
  try {
    Logger.log("Memulai GACOR HUNTER V5.0 EOD Scan (Synchronized)...");
    const candidates = [];
    ...
    for (let start = 0; start < WATCHLIST.length; start += CONFIG.BATCH_SIZE) {
      ...
      for (let j = 0; j < responses.length; j++) {
        ...
        try {
          ...
          // DIPERBAIKI: Optional Chaining agar tahan banting jika data tidak lengkap dari Yahoo
          ...
          for (let k = 0; k < quote.close.length; k++) {
            if (quote.close[k] != null && quote.volume[k] != null) {
              validData.push({
                close: Number(quote.close[k]),
                volume: Number(quote.volume[k]),
                high: Number(quote.high[k] || quote.close[k]),
                low: Number(quote.low[k] || quote.close[k])
              });
            }
          }

          // Minimal 120 candle (sesuai engine V5.0 di frontend)
          ...
          // ----------------------------------------------------
          // FILTER BASIC: Hindari saham mati / kurang liquid
          // ----------------------------------------------------
          const ema5Volume = calcEMA(volumes, 5);
          const avgTransactionValue = ema5Volume * currClose;
          
          if (ema5Volume < CONFIG.MIN_VOLUME_EMA5 && avgTransactionValue < CONFIG.MIN_AVG_DAILY_VALUE) {
            continue; 
          }
          if (previousVolume <= ema5Volume) continue; 
          
          // ----------------------------------------------------
          // V5.0 ADVANCED CALCULATION (Persis seperti index.html)
          // ----------------------------------------------------
          const return1d = prevClose ? ((currClose - prevClose) / prevClose) * 100 : 0;
          const close5d = closes[closes.length - 6];
          const return5d = close5d ? ((currClose - close5d) / close5d) * 100 : 0;

          const e5 = emaSeries(closes, 5);
          const e21 = emaSeries(closes, 21);
          const e34 = emaSeries(closes, 34);
          const e99 = emaSeries(closes, 99);
          
          const ema5 = e5[e5.length - 1];
          const ema21 = e21[e21.length - 1];
          const ema34 = e34[e34.length - 1];
          const ema99 = e99[e99.length - 1];
          ...
          const volRatio = avgVol20 ? previousVolume / avgVol20 : 0;
          ...
          const piv = getPivots(highs, lows, 2);
          const structure = buildStructure(piv);
          
          const recentH = piv.highPivots.slice(-10).map(x => x.price);
          const recentL = piv.lowPivots.slice(-10).map(x => x.price);
          ...
          const win = Math.min(80, closes.length);
          ...
          const swingHigh = highs[hiI], swingLow = lows[loI];
          const fib = fibMap(swingLow, swingHigh, structure.bullish || currClose > ema21);
          const fibS = fib ? nearestBelow(Object.values(fib), currClose) : 0;
          const fibR = fib ? nearestAbove(Object.values(fib), currClose) : 0;
          ...
          const emaBull = ema5 > ema21 && ema21 > ema34;
          const emaBear = ema5 < ema21 && ema21 < ema34;
          ...
          const dS = classicS ? Math.abs(currClose - classicS) / currClose : 1;
          const dR = classicR ? Math.abs(classicR - currClose) / currClose : 1;
          ...
          Logger.log(`Error parsing ${ticker}: ${err.message}`);
        }
      }
      Utilities.sleep(CONFIG.BATCH_SLEEP_MS);
    }

    // Urutkan berdasarkan Skor Tertinggi V5.0
    candidates.sort((a, b) => {
    ...
    Logger.log("EOD Scan Berhasil (V5 Sync). Total Lolos Filter: " + candidates.length);
  } finally {
    lock.releaseLock();
  }
}
...
// ============================================================
// 6. API ENDPOINT (Merespon Request dari HTML)
// ============================================================
function doGet(e) {
  ...
  if (params.action === "heartbeat") {
    return ContentService.createTextOutput(JSON.stringify({
      status: "success",
      totalUsers: Math.floor(Math.random() * 5) + 15, 
      onlineUsers: Math.floor(Math.random() * 3) + 2
  ...
  return ContentService.createTextOutput(JSON.stringify({
    status: "success",
    engine: "GACOR HUNTER V5.0 BACKEND (SYNCED)",
...
function setupDailyTrigger() {
  ...
  // DIPERBAIKI: Mengubah jadwal eksekusi ke jam 18:00 WIB agar data EOD dari Yahoo Finance sudah final dan lengkap
  ScriptApp.newTrigger("runEODScan").timeBased().everyDays(1).atHour(18).inTimezone("Asia/Jakarta").create();
  
  Logger.log("Trigger EOD Scan berhasil disetup.");
}
